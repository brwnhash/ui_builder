...
class FigmaPy:
    ...
    def parse_file_data(self,key,data,parent=None):
        if data!=None:
    ...
    def get_file_components(self, file_key,id=None):
        data=None
        if id:
            data = self.api_request(f'components/{id}', method='get')
        else:
            data = self.api_request(f'files/{file_key}/components', method='get')
        
        return data

    def get_file_componentsets(self,file_key,id=None):
        data=None
        if id:
            data = self.api_request(f'files/{file_key}/component_sets?ids={id}', method='get')
        else:
            data = self.api_request(f'files/{file_key}/component_sets', method='get')
